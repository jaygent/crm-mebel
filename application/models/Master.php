...
public function iszhurnalExists($id) {
    ...
    switch ($post['type']) {
      case '0':
      $params=['search'=>"%".$post['ref']."%", 'year'=>$post['data'],];
        $dat=$this->db->row('SELECT id_zakaza FROM zakaz WHERE statuss<4 AND year=:year and nomer_zakaza LIKE :search OR nomer_lena LIKE :search', $params);
        $d='(';
                        foreach($dat as $val){
                         $d.=$val['id_zakaza'].',';
                        }  $dat=substr($d,0,-1);
      ...
      case '1':
        $params=['search'=>"%".$post['ref']."%",'year'=>$post['data'],];
        $dat=$this->db->row('SELECT id_zakaza FROM zakaz WHERE statuss<4 AND year=:year and klient LIKE :search or phone1 LIKE :search OR phone2 LIKE :search', $params);
        $d='(';
                        foreach($dat as $val){
                         $d.=$val['id_zakaza'].',';
                        }  $dat=substr($d,0,-1);
    ...
    if(!empty($dat)){
      $par=['id'=>$dat,];
    $zakaz=$this->db->row('SELECT * FROM zakaz WHERE statuss<4 AND id_zakaza IN'.$dat);
    foreach($zakaz as $keyss=>$valss){
        $data[$keyss]=$valss;
    $params = ['id_zakaza' => $valss['id_zakaza'],];
    $mebel=$this->db->row('SELECT * FROM mebel WHERE id_zakaza=:id_zakaza', $params);
