        ...
        <div class="card mb-3">
            <div class="card-header">Заказы в работе Журнал <?if(!empty($idmaster)){ foreach ($master as $val) {
                  if($val['id_master']==$idmaster){
                            echo $val['name'].'<a class="btn bg-danger" href="/master/year/'.$year.'/yearworks/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/master/year/'.$year.'/yearnooplata/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        }
                        if($idmaster==1){  echo 'Общий <a class="btn bg-danger" href="/master/year/'.$year.'/yearworks/1" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/master/year/'.$year.'/yearnooplata/1" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        } ?></div>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <?php if(!empty($maste)){
                        echo '<a href="/master/year/'.$year.'/yearposts/1" class="btn btn-primary" style="margin:0 20px;">Общий журнал мастеров</a>';
                        foreach ($maste as $val) {
                          if($val['proff']==='Выездной мастер'){
                            echo '<a href="/master/year/'.$year.'/yearposts/'.$val['id_master'].'" class="btn btn-primary" style="margin:0 20px;">Журнал выездного мастера '.$val['name'].'</a>';
                          }
                        }}?>
