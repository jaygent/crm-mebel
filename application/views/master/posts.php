        ...
        <div class="card mb-3">
            <div class="card-header">Журнал <?if(!empty($idmaster)){ foreach ($master as $val) {
                        if($val['id_master']==$idmaster){
                            echo $val['name'].'<a class="btn bg-info" href="/master/r/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">В ракушке</a><a class="btn bg-danger" href="/master/works/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/master/nooplata/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        }
                        if($idmaster==1){  echo 'Общий <a class="btn bg-info" href="/master/r/1" style="border: 2px solid #fff; color: #fff;">В ракушке</a> <a class="btn bg-danger" href="/master/works/1" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/master/nooplata/1" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        } ?></div>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <?php if(!empty($maste)){
                        echo '<a href="/master/posts/1" class="btn btn-primary" style="margin:0 20px;">Общий журнал мастеров</a>';
                        foreach ($maste as $val) {
                          if($val['proff']==='Выездной мастер'){
                            echo '<a href="/master/posts/'.$val['id_master'].'" class="btn btn-primary" style="margin:0 20px;">Журнал выездного мастера '.$val['name'].'</a>';
                          }
                        }}?>
                ...
                if(!empty($value['master'])){
                  ...
                  switch ($value['statuss']) {
                      case -1:
                     $sdacha='<a href="/master/inwork/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-secondary">Взять в работу</a>';
                      break;
                    case '0':
                     $sdacha='<a href="/master/gotov/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-secondary">Заказ Готов</a>';
                      break;
                    case '1':
                     $sdacha='<a href="/master/sdacha/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-primary">Сдать заказ</a>';
                      break;
                  }
                  switch ($value['statuss']) {
                    case '2':
                     $edit='';
                      break;
                    default:
                     $edit='<a href="/master/edit/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-primary">Ред</a>';
                      break;
                  }
