        ...
        <div class="card mb-3">
            ...
            <div class="card-header">Журнал <?if(!empty($idmaster)){ foreach ($master as $val) {
                        if($val['id_master']==$idmaster){
                            echo $val['name'].'<a class="btn bg-info" href="/manager/r/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">В ракушке</a><a class="btn bg-danger" href="/manager/works/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/manager/nooplata/'.$val['id_master'].'" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        }
                        if($idmaster==1){  echo 'Общий <a class="btn bg-info" href="/manager/r/1" style="border: 2px solid #fff; color: #fff;">В ракушке</a> <a class="btn bg-danger" href="/manager/works/1" style="border: 2px solid #fff; color: #fff;">В работе</a><a class="btn bg-warning" href="/manager/nooplata/1" style="border: 2px solid #fff; color: #fff;">Готов-Не оплачен</a>';
                          }
                        } ?></div>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <?php if(!empty($maste)){
                        echo '<a href="/manager/posts/1" class="btn btn-primary" style="margin:0 20px;">Общий журнал мастеров</a>';
                        foreach ($maste as $val) {
                          if($val['proff']==='Выездной мастер'){
                            echo '<a href="/manager/posts/'.$val['id_master'].'" class="btn btn-primary" style="margin:0 20px;">Журнал выездного мастера '.$val['name'].'</a>';
                          }
                        }}?>
                ...
                <?php     foreach ($list as $value) {
                    ...
                    switch ($value['statuss']) {
                          case -1:
                        if($value['lena_status']==1){$lena_staatus='';}
                    ...
                    echo '<td aria-label="№">'.$value['nomer_zakaza'].'<hr><p style="color:red;">'.$value['nomer_lena'].'</p></td>';
                        ...
                        echo '<td aria-label="Данные Клиента" style="word-break: break-word;">'.$value['klient'].'<br>'.$value['phone1'].' - Заказ '.$send_m1.'/ Отзыв - '.$value['send_m1_2'].'<br>'.$value['phone2'].'</td>';
                ...
                if(!empty($value['master'])){
                  ...
                  switch ($value['statuss']) {
                      case -1:
                     $sdacha='<a href="/manager/inwork/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-secondary">Взять в работу</a>';
                      break;
                    case '0':
                     $sdacha='<a href="/manager/gotov/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-secondary">Заказ Готов</a>';
                      break;
                    case '1':
                     $sdacha='<a href="/manager/sdacha/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-primary">Сдать заказ</a>';
                      break;
                  }
                  switch ($value['statuss']) {
                    case '2':
                     $edit='';
                      break;
                    default:
                     $edit='<a href="/manager/edit/'.$idmaster.'/'.$value["id_zakaza"].'" class="btn btn-primary">Ред</a>';
                      break;
                  }
                  echo '<td aria-label="Ред.">'.$bt.'</br>'.$sdacha.''.$edit.'</td></tr>';
                }?>
                            </table>
                            <?php echo $pagination; ?>
