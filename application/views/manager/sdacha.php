            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                     ...
                     <form action="/manager/sdacha/<? echo $idmaster ?>/<?php echo $data[0]['id_zakaza']; ?>" method="post">
                         ...
                         <div class="form-group">
                                ...
                                <select name="master" required><?php if($idmaster!==1){foreach($master as $m){if($m['id_master']==$idmaster){echo "<option value='".$m['id_master']."' >".$m['name']."</option>";}}}else{foreach($master as $m){if($m['proff']==='мастер'){echo "<option value='".$m['id_master']."' >".$m['name']."</option>";}}} ?></select>
                        </div>
                         <div class="form-group">
                                <label>Диспетчер сдавший заказ</label>
                                <select name="dispetcher" required><?php foreach($master as $m){if($m['id_master']==$_SESSION['id']){echo '<option value="'.$_SESSION['id'].'">'.$m['name'].'</option>';}} ?></select>
                        </div>
